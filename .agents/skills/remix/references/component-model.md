...
function ThemeProvider(handle: Handle<{ children?: RemixNode }, Theme>) {
    ...
    return () => (
        <div>
            <button
                mix={on("click", () => theme.setValue(theme.value === "light" ? "dark" : "light"))}
            >
                Toggle
            </button>
            {handle.props.children}
...
Use native `addEventListener` for global targets such as `window` and `document`. Schedule
browser-only setup with `handle.queueTask()` and pass `handle.signal` so the listener is removed
