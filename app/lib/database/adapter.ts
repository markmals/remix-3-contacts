...
export class BunSqlDatabaseAdapter implements DatabaseAdapter {
    ...
    async execute(
        ...
        if (isReaderOperation(request.operation)) {
            ...
            return {
                rows,
                affectedRows: isWriteKind(request.operation.kind)
                    ? rows.length
                    : undefined,
    ...
    async acquireMigrationLock(): Promise<void> {
        if (this.dialect === "postgres") {
            ...
            await this.#sql.unsafe(
                "SELECT GET_LOCK('remix_data_table_migration', -1)",
            );
        }
    }
    ...
    async releaseMigrationLock(): Promise<void> {
        if (this.dialect === "postgres") {
            await this.#sql.unsafe(
                "SELECT pg_advisory_unlock(72616461)",
            );
        } else if (this.dialect === "mysql") {
            await this.#sql.unsafe(
                "SELECT RELEASE_LOCK('remix_data_table_migration')",
            );
        }
    }
